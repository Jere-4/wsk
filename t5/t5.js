...
const proceedWithLocation = (userLat, userLon) => {
  ...
  const restaurantWithDistance = clean.map((rivi) => {
    const [lon, lat] = rivi.location.coordinates;

    distance = calculateDistance(userLon, userLat, lon, lat);

    return {...rivi, distance};
  });
...
const onError = () => {
  console.log('Virhe');
  yourLoc.textContent =
    'Sijainnin saaminen ei onnistunut, käytetään oletuksena Helsinki';
  proceedWithLocation(lat, lon);
};
